...
**重要**：必须使用已安装neo4j驱动的Python环境。

如果系统默认Python未安装neo4j，请使用Anaconda环境：
```bash
# 使用Anaconda的Python（已安装neo4j驱动）
D:\ProgramData\anaconda3\python.exe .trae/skills/memory-graph/.memory/memory_ops.py retrieve "test"
```
...
4. 如果提示Neo4j不可用，请检查Python环境是否正确
...
2. 调用 retrieve_memory(query) 检索相关记忆
3. 将检索结果插入上下文
```

**注意**：检索时会自动应用遗忘曲线，长时间未访问的记忆得分会降低。

### 2. 对话后保存 (After Conversation)

对话结束后（或用户表示满意时），**必须执行记忆提取和保存**：
...
4. 调用 save_memory() 保存到图谱
```

### 3. 记忆维护 (Memory Maintenance)

**每隔一段时间（建议每10次对话后）**，**必须执行记忆维护**：

```
记忆维护流程:
1. 调用 consolidate_memories() - 巩固重要记忆
2. 调用 cleanup_memories() - 清理过期短期记忆
3. 调用 organize_memories() - 整理相似记忆
```

**遗忘机制说明**：
- **短期记忆**：24小时未访问自动衰减，超过时间自动清理
- **长期记忆**：访问频率低时得分降低，但不会自动删除
- **记忆强化**：频繁访问的记忆自动转为长期记忆并提高权重
